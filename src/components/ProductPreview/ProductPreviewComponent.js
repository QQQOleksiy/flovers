...
const ProductPreview = ({product}) => {
    ...
    return (
        <div className={css.product_pre} onClick={() => navigate(`/item/${id}`)}>
           <img src={`https://www.sinowrap.online/static/${main_photo_path[0]}`} className={css.product_big_image} alt=''/>
           <div className={css.product_container_small_images}>
                <img src={`https://www.sinowrap.online/static/${main_photo_path[1]}`} className={css.product_small_image} alt=''/>
                <img src={`https://www.sinowrap.online/static/${main_photo_path[2]}`} className={css.product_small_image} alt=''/>
                <div className={css.product_small_image} style={{position: 'relative'}}>
                    <span>+{colors.length} цветов</span>
                    <img src={`https://www.sinowrap.online/static/${main_photo_path[3]}`} className={css.product_small_image} alt=''/>
                </div>
           </div>
